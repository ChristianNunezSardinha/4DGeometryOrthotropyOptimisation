...
import numpy as np
# This will be a file that contains all of the necessary variables for abaqus_functions.py.
# CreatePart()
MODEL_NAME = 'Model-1'
PART_NAME = 'Part-1'
POINT_1 = (0.0, 0.0)
...
MAGNITUDE_TEMPERATURE_END = 100.0
# CreateJob(submit_job=True)
executeOnCaeStartup()
def CreatePart(
...
def CreateCompositeLayup(MODEL_NAME, PART_NAME, COMPOSITE_LAYUP_NAME, ANGLEPLY_PASSIVE, ANGLEPLY_ACTIVE):
    ...
    compositeLayup.CompositePly(suppressed=False, plyName='Ply-2', region=region2, 
        material=PASSIVE_MATERIAL_NAME, thicknessType=SPECIFY_THICKNESS, thickness=1.0, 
...
def CreateBoundaryConditions(MODEL_NAME, ASSEMBLY_NAME, STEP_NAME, STEP_INITIAL_NAME):
    a = mdb.models[MODEL_NAME].rootAssembly
    f1 = a.instances[ASSEMBLY_NAME].faces
    faces1 = f1.getSequenceFromMask(mask=('[#1 ]', ), )
    ...
    mdb.models[MODEL_NAME].boundaryConditions['BC-1'].setValuesInStep(
        stepName=STEP_NAME, magnitude=100.0)

def CreateJob(MODEL_NAME, JOB_NAME, submit_job = True):
...
for i in range(0,ITERATION_NUMBER):
    JOB_NAME = 'Job-' + str(i)
    ANGLEPLY_ACTIVE_LIST =  np.arange(0,100,10)
    ANGLEPLY_ACTIVE = ANGLEPLY_ACTIVE_LIST[i]
    CreateCompositeLayup(MODEL_NAME, PART_NAME, COMPOSITE_LAYUP_NAME, ANGLEPLY_PASSIVE, ANGLEPLY_ACTIVE)
    CreateMesh(MODEL_NAME, ASSEMBLY_NAME)
    CreateStep(MODEL_NAME, ASSEMBLY_NAME, STEP_INITIAL_NAME)
    CreateBoundaryConditions(MODEL_NAME, ASSEMBLY_NAME, STEP_NAME, STEP_INITIAL_NAME)
    CreateJob(MODEL_NAME, JOB_NAME, submit_job = False)
